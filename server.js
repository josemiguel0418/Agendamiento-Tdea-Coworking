...
app.post("/registro", async (req, res) => {
  ...
  db.run("INSERT INTO usuarios(nombre,documento,password,rol) VALUES(?,?,?,'usuario')",
    [nombre, documento, hash],
    (err) => {
...
app.post("/login", (req, res) => {
  ...
  console.log("Login intento:", documento, password);
...
app.get("/admin/usuarios", (req, res) => {
  ...
  db.all("SELECT id, nombre, documento, rol FROM usuarios", [], (err, rows) => {
    if (err) res.json([]);
    else res.json(rows);
  });
});
